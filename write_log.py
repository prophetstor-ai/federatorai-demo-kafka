...
class WriteLog:
    ...
    def get_pod_limit(self, pod_name):
        ...
        if output:
            try:
                ...
                if output.get("spec", {}).get("containers", [])[0].get("resources"):
                    cpu_limit_mcore = output.get("spec", {}).get("containers", [])[0].get("resources").get("limits").get("cpu", "0m")
                ...
                if output.get("spec", {}).get("containers", [])[0].get("resources"):
                    memory_limit_mb = output.get("spec", {}).get("containers", [])[0].get("resources").get("limits").get("memory", "0Mi")
    ...
    def get_pod_reason(self, pod_name):
        ...
        if output:
            ...
            if output.get("status").get("containerStatuses")[0].get("lastState"):
                terminated = output.get("status").get("containerStatuses")[0].get("lastState").get("terminated").get("reason")
    ...
    def calculate_overlimit(self, algo, time_count):
        ...
        for i in range(data_count):
            ...
            self.get_metrics()
            self.get_status()
            print "--- %s start to collect data at %d/%d interval(in 30 sec) ---" % (algo.split("_")[0], i, data_interval * 2)
            for deployment in self.app_list.keys():
                cpu_limit = 0
                memory_limit = 0
                total_cpu = 0
                ...
                data = [algo_name, total_cpu, total_cpu_limit, total_memory, total_memory_limit, cpu_count, memory_count, num_replica, restart, total_status]
